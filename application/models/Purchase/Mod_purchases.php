...
class Mod_purchases extends CI_model {
	...
	function get_purchases_limit($limit,$offset){
		return $this->db->query("SELECT * FROM purchases WHERE status = 'Pembelian' LIMIT $limit OFFSET $offset ");
	}
	function get_purchases_limit_src($limit,$offset,$src){
		return $this->db->query("SELECT * FROM purchases WHERE status = 'Pembelian' AND (kode LIKE '%$src%' OR item LIKE '%$src%' OR kode_item LIKE '%$src%') LIMIT $limit OFFSET $offset ");
	}
