...
class Mod_purchases extends CI_model {
	...
	function get_purchases(){
		return $this->db->query("SELECT *,(SELECT name FROM suppliers WHERE kode = a.supplier ) AS supplier_name FROM purchases AS a WHERE status = 'Pembelian' AND finish = 1");
	}
	...
	function get_purchases_limit($limit,$offset){
		return $this->db->query("SELECT *,(SELECT name FROM suppliers WHERE kode = a.supplier ) AS supplier_name FROM purchases AS a WHERE status = 'Pembelian' AND finish = 1 ORDER BY created_at DESC LIMIT $limit OFFSET $offset ");
	}
	function get_purchases_limit_src($limit,$offset,$src){
		return $this->db->query("SELECT *,(SELECT name FROM suppliers WHERE kode = a.supplier ) AS supplier_name FROM purchases AS a WHERE status = 'Pembelian' AND finish = 1 AND (kode LIKE '%$src%' OR item LIKE '%$src%' OR kode_item LIKE '%$src%') ORDER BY created_at DESC LIMIT $limit OFFSET $offset ");
	}
